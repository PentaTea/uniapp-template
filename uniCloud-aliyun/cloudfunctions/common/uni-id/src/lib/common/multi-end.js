import { db, userCollection, PublicErrorCode, uniqueUserParam } from '../utils/config'

import { getObjectValue, getType } from '../../share/index'

const dbCmd = db.command

async function checkCanAuthorize({ uid, dcloudAppidList } = {}) {
  if (!uid) {
    return {
  ...
  if (
    !dcloudAppidList ||
    dcloudAppidList.length === 0 ||
    dcloudAppidList.some((item) => item === undefined)
  ) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
  ...
  switch (userQuery.length) {
    ...
    case 1:
      query = dbCmd.or([
        dbCmd.and([userQuery[0], appidQuery]),
        dbCmd.and([userQuery[0], appidNotExistQuery]),
      ])
      break
    default:
      query = dbCmd.or([
        dbCmd.and([dbCmd.or(userQuery), appidQuery]),
        dbCmd.and([dbCmd.or(userQuery), appidNotExistQuery]),
      ])
      break
  }
...
async function setAuthorizedAppLogin({ uid, dcloudAppidList } = {}) {
...
async function authorizeAppLogin({ uid, dcloudAppid } = {}) {
  const checkCanAuthorizeRes = await checkCanAuthorize.bind(this)({
    uid,
...
async function forbidAppLogin({ uid, dcloudAppid } = {}) {
  if (!uid) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
...
export { setAuthorizedAppLogin, authorizeAppLogin, forbidAppLogin }
