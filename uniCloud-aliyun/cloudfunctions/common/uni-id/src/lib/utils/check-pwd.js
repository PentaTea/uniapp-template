import { getType } from '../../share/index'
// user对象
function checkPwd(user, password) {
  ...
  const { password: pwdInDB, password_secret_version: passwordVersion } = user
  ...
  if (passwordSecretType === 'string') {
    const { passwordHash } = this.encryptPwd(password, { value: passwordSecret })
  ...
  const { passwordHash: previousPasswordHash } = this.encryptPwd(password, previousSecret)
  if (previousPasswordHash === pwdInDB) {
    ...
    if (previousSecret !== currentSecret) {
      const { passwordHash: currentPasswordHash, version: currentVersion } = this.encryptPwd(
        password,
        currentSecret
      )
