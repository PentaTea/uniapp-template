...
async function login({ username, password, queryField = [], needPermission }) {
  ...
  const { usernameToLowerCase, emailToLowerCase, passwordErrorLimit, passwordErrorRetryTime } =
    this._getConfig()
  ...
  // 清理无用记录
  loginIPLimit = loginIPLimit.filter(
    (item) => item.last_error_time > Date.now() - passwordErrorRetryTime * 1000
  )
  ...
  if (checkPwdRes.code === 0) {
    ...
    // 迁移用户密码
    const { passwordHash, passwordVersion } = checkPwdRes
