...
WaterWave.prototype.init = function(opt) {
  ...
  style.innerHTML =
    ...
    '-khtml-opacity:0.5;' +
    '-webkit-opacity:0.5;' +
    '-webkit-animation: rippleMove 1s ease-out forwards;}' +
    '-moz-animation: rippleMove 1s ease-out forwards;}' +
    '-ms-animation: rippleMove 1s ease-out forwards;}' +
    '-o-animation: rippleMove 1s ease-out forwards;}' +
    'animation: rippleMove 1s ease-out forwards;}' +
    '@-webkit-keyframes rippleMove{' +
    'from{opacity: .5;-webkit-transform:scale(1);}' +
    'to{opacity: 0;-webkit-transform:scale(' +
    this.settins.scale +
    ');}}' +
    '@-moz-keyframes rippleMove{' +
    'from{opacity: .5;-moz-transform:scale(1);}' +
    'to{opacity: 0;-moz-transform:scale(' +
    this.settins.scale +
    ');}}' +
    '@-ms-keyframes rippleMove{' +
    'from{opacity: .5;-ms-transform:scale(1);}' +
    'to{opacity: 0;-ms-transform:scale(' +
    this.settins.scale +
    ');}}' +
    '@-o-keyframes rippleMove{' +
    'from{opacity: .5;-o-transform:scale(1);}' +
    'to{opacity: 0;-o-transform:scale(' +
    this.settins.scale +
    ');}}' +
    '@keyframes rippleMove{' +
    'from{opacity: .5;transform:scale(1);}' +
    'to{opacity: 0;transform:scale(' +
    this.settins.scale +
    ');}}'
...
WaterWave.prototype.found = function(obj, event) {
  var ev = event
  ...
  span.style.marginTop = event.layerY - this.settins.WH / 2 + 'px'
