...
import { getDistinctArray, getType } from '../../share/index'
...
async function getPermissionByUid({ uid }) {
  const roleRecord = await userCollection
    .aggregate()
    .match({
      _id: uid,
    })
...
async function unbindRole({ uid, roleList }) {
  if (typeof roleList === 'string') {
    roleList = [roleList]
  }
...
async function unbindPermission({ roleID, permissionList }) {
  if (typeof permissionList === 'string') {
    permissionList = [permissionList]
  }
  await roleCollection
...
async function addRole({ roleID, roleName, comment, permission = [] }) {
  if (!roleID) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
...
async function getRoleList({ limit = 20, offset = 0, needTotal = true }) {
  ...
  if (needTotal) {
    const { total } = await roleCollection
      .where({
...
async function updateRole({ roleID, roleName, comment, permission }) {
  if (!roleID) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
...
async function addPermission({ permissionID, permissionName, comment }) {
  if (!permissionID) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
...
async function getPermissionList({ limit = 20, offset = 0, needTotal = true }) {
  ...
  if (needTotal) {
    const { total } = await permissionCollection
      .where({
        _id: dbCmd.exists(true),
      })
      .count()
...
async function getPermissionInfo(permissionID) {
  const permissionRecord = await permissionCollection
    .where({
      permission_id: permissionID,
    })
    .get()
...
async function updatePermission({ permissionID, permissionName, comment }) {
  if (!permissionID) {
    return {
      code: PublicErrorCode.PARAM_REQUIRED,
      messageValues: {
