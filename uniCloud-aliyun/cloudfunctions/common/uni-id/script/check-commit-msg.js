...
const commitRE =
  /^(\d+\.\d+\.\d+(-(alpha|beta|rc.\d+))?)|((revert: )?(feat|fix|polish|docs|style|refactor|perf|test|workflow|ci|chore|types|build)(\(.+\))?!?: .{1,50})/

if (!commitRE.test(message)) {
  console.log('\u001b[41;37mGit提交信息不符合规范\u001b[0m')
  console.log('提交信息需形如：type(scope?): subject')
  console.log(
    'type可取值为：revert|feat|fix|polish|docs|style|refactor|perf|test|workflow|ci|chore|types|build'
  )
