...
export default async function loginByApple({
  ...
  const appleConfig =
    config && config['app-plus'] && config['app-plus'].oauth && config['app-plus'].oauth.apple
  ...
  fullName =
    nickName ||
    (fullName && Object.keys(fullName).length > 0 ? fullName.familyName + fullName.givenName : '')

  const { code, msg } = await platformApi
    .initApple({
  ...
  const { iss, sub, aud, email: checkEmail } = msg
  ...
  const nickname = fullName || `User-${checkEmail.split('@')[0]}`
