...
checkboxReplace = function(md, options, Token) {
  ...
  createTokens = function(checked, label, Token) {
     ...
     */
    if (options.divWrap) {
      token = new Token('checkbox_open', 'p', 1)
      token.attrs = [['class', options.divClass + (checked ? ' checked' : '')]]
      nodes.push(token)
    }
     ...
     */
    token = new Token('span_open', 'span', 1)
    token.attrs = [['for', id]]
    nodes.push(token)
     ...
     * closing tags
     */
    nodes.push(new Token('span_close', 'span', -1))

    if (options.divWrap) {
      nodes.push(new Token('checkbox_close', 'p', -1))
    }
    return nodes
  }
