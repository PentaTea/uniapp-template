import { camel2snakeJson, snake2camelJson, getOffsetDate, getFullTimeStr } from '../../share/index'
...
export default class AlipayBase {
  constructor(options = {}) {
    ...
    const privateKeyType = this.options.keyType === 'PKCS8' ? 'PRIVATE KEY' : 'RSA PRIVATE KEY'

    this.options.privateKey = this._formatKey(this.options.privateKey, privateKeyType)
    if (this.options.alipayPublicKey) {
      this.options.alipayPublicKey = this._formatKey(this.options.alipayPublicKey, 'PUBLIC KEY')
    }
  }
  ...
  async _exec(method, params = {}, option = {}) {
    ...
    if (response) {
      // 按字符串验签
      const validateSuccess = option.validateSign
        ? this._checkResponseSign(data, responseKey)
        : true
      if (validateSuccess) {
        ...
        const msg = response.sub_code
          ? `${response.sub_code} ${response.sub_msg}`
          : `${response.msg || 'unkonwn error'}`
        throw new Error(msg)
      } else {
