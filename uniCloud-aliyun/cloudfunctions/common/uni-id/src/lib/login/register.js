...
async function register(user) {
  ...
  const { usernameToLowerCase, emailToLowerCase } = this._getConfig()
  ...
  uniqueParam.forEach((item) => {
    ...
    if (paramValue) {
      if (
        (item.name === 'username' && usernameToLowerCase) ||
        (item.name === 'email' && emailToLowerCase)
      ) {
  ...
  const { username, email, mobile, myInviteCode, needPermission, autoSetDcloudAppid = true } = user
  ...
  const { passwordHash, version } = this.encryptPwd(orginPassword)
  ...
  user.username_raw = username_raw
