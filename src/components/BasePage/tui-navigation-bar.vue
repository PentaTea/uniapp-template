  ...
  <div>
    ...
    >
      ...
      <view
        class="absolute"
        :style="{
          width: capsule ? canuse + 'px' : '750rpx',
    ...
    <div
      v-if="isFixed && !isOpacity && !noPlaceholder"
      :style="{
        height: height + 'px',
      }"
    ></div>
...
export default {
  ...
  created() {
    ...
    uni.getSystemInfo({
      success: (res) => {
        ...
        this.canuse = this.left //- (this.width - obj.right)
        if (this.isImmersive) {
  ...
  methods: {
    ...
    opacityChange() {
      ...
      const scroll = this.scrollTop <= 1 ? 0 : this.scrollTop
      const opacity = scroll / this.scrollH
      if (
        (this.opacity >= this.maxOpacity && opacity >= this.maxOpacity) ||
